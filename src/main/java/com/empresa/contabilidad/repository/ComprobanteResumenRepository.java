...
@Repository
public interface ComprobanteResumenRepository extends JpaRepository<Comprobante, ComprobanteId>, JpaSpecificationExecutor<Comprobante> {
    ...
    @Query("SELECT new com.empresa.contabilidad.dto.ComprobanteResumenDto(" +
            ...
            "c.glosa, c.codTipoComprobanteGenerado, c.estadoSistema, c.codEmisionCheqhe, c.fechaSistema, c.descrMontoTotal,0,0) " +
            "FROM Comprobante c " +
    ...
    @Query("SELECT new com.empresa.contabilidad.dto.ComprobanteResumenDto(" +
            ...
            "AND (CAST(:fechaComprobante AS text) IS NULL OR CAST(c.fechaComprobante AS date) = CAST(:fechaComprobante AS date)) " +
            ...
            "ORDER BY c.codGestion, c.codComprobante")
    Page<ComprobanteResumenDto> findAllResumenFiltered(
